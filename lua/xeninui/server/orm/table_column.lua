...
do
  ...
  local _base_0 = {
    ...
    buildString = function(self)
      ...
      if (!isMySQL and unsigned) then str = str .. tostring(unsigned) .. " "
      end
      str = str .. type
      ...
      if self._foreign then local __lauxi0 = self._foreign
        assert(__lauxi0 ~= nil, "cannot destructure nil value")
        local tbl, id = __lauxi0.tbl, __lauxi0.id
        str = str .. " FOREIGN KEY REFERENCES " .. tostring(tbl) .. "(" .. tostring(id) .. ")"
      end
      if self._increments then str = str .. (self.isMySQL and " AUTO_INCREMENT" or " AUTOINCREMENT")
      end
      if (!self._nullable) then str = str .. " NOT NULL"
      end
      if self._default then local __lauxi1 = self._default
        assert(__lauxi1 ~= nil, "cannot destructure nil value")
        local value, surround = __lauxi1.value, __lauxi1.surround
      ...
      if self._onUpdate then str = str .. (" ON UPDATE " .. self._onUpdate)
      end
