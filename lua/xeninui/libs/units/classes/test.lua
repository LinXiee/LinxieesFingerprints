      ...
      if result:isSuccess() then
        if istable(result:getReturn()) then
          result:getReturn():next(function()
            p:resolve(result:getMessagePrint())end, function(err)
            p:reject(result:getMessagePrint())end)
        else
...
function XeninUI:Test(script, print)
  if print == nil then print = true
  end
  local p = XeninUI.Promises.new()
  local tbl = XeninUI.Scripts:get(script)
  if (!tbl) then
    if print then MsgC(XeninUI.Theme.Red, "That script doesn't exist\n")end
  ...
  for i, v in ipairs(tests) do
    table.insert(promises, v:run())
  end

  XeninUI.Promises.all(promises):next(function(results)
    local time = 0
    local passes = 0
    local fails = 0
    local size = 0
    ...
    for i, v in ipairs(results) do
      time = time + v.executionTime
      passes = passes + v.successes
      fails = fails + v.errors
      size = size + v.size

      table.insert(msgs, v.msgs)
    end

    local successRate = (passes / size) * 100
    if print then
...
concommand.Add("xenin_test", function(ply, cmd, args)
  ...
  XeninUI:Test(args[1], !tobool(args[2])):next(function(result) end, function(err)
    ErrorNoHalt(err .. "\n")end)
end)
