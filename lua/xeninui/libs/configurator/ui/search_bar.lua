...
function PANEL:CreatePopup(results)
  ...
  for i = 1, size do
    ...
    panel.Paint = function(pnl, w, h)
      ...
      XeninUI:DrawShadowText(result.desc, "Xenin.Configurator.Admin.SearchBar.Subtitle", h, h / 2, Color(179, 179, 179), TEXT_ALIGN_LEFT, TEXT_ALIGN_TOP, 1, 150)
...
function PANEL:Find(text, search)
  return text:find(search, nil, true)
end
...
function PANEL:SearchTraits(text)
  if (self:ReachedSearchLimit()) then return end

  local cat = "Trait"
  local id = "traits"
  local tbl = XeninTTT.Weapons.Traits:getAll()
  for i, v in pairs(tbl) do
    if (self:ReachedSearchLimit()) then return end

    local trait = v()
    local name = trait:getName()
    local desc = trait:getDisplayDesc()
    if self:Find(trait:getId(), text) then
      self:AddToSearch(name, desc, cat, id)

      continue
    end
    if self:Find(name:lower(), text) then
      self:AddToSearch(name, desc, cat, id)
...
function PANEL:ReachedSearchLimit()
