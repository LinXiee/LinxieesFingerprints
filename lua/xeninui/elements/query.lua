...
function PANEL:Init()
  ...
  self.background.accept.DoClick = function(pnl)
    if self:onAccept(pnl, self.background.input:GetText(), self.background.option.sid64) then
      return -- Return true, for prevent close menu.
    end
  ...
  self.background.decline.DoClick = function(pnl)
    if self:onDecline(pnl, self.background.input:GetText(), self.background.option.sid64) then
      return -- Return true, for prevent close menu.
    end
